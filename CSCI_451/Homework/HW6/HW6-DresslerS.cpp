...
//Thread to read from the hw6-1.in
void * thread1(void * arg){
    pthread_cond_wait(&c1, &m1);
    cout << "1st TID-"<<pthread_self() << endl;
    int thread_num = 1;
    cout <<"---------------------------------"<<endl;
    cout << "Opening File 1 for Input..."<<endl;
    cout <<"---------------------------------"<<endl;
    FILE * filein1 = fopen("hw6-1.in","r");
...
//Thread to read from hw6-2.in
void * thread2(void * arg){
    pthread_cond_wait(&c2, &m2);
    cout << "2nd TID-"<<pthread_self() << endl;
    int thread_num = 2;
    cout <<"---------------------------------"<<endl;
    cout << "Opening File 2 for Input..."<<endl;
    cout <<"---------------------------------"<<endl;
    FILE * filein2 = fopen("hw6-2.in","r");
...
//Thread to read from hw6-3.in
void * thread3(void * arg){
    pthread_cond_wait(&c3, &m3);
    cout << "3rd TID-"<<pthread_self() << endl;
    int thread_num = 3;
    cout <<"---------------------------------"<<endl;
    cout << "Opening File 3 for Input..."<<endl;
    cout <<"---------------------------------"<<endl;

    FILE * filein3 = fopen("hw6-3.in","r");
...
void get_next_char(FILE * in, int tnum){
    if(tnum == 1){
        while(1){
                if((fscanf(in, "%c\n",&resource)) != EOF){
                    cout << "Reading char from file: \"hw6-1.in\"" << endl;
                    print = true;
    ...
    if(tnum == 2){
        while(1){
                if((fscanf(in, "%c\n",&resource)) != EOF){
                    cout << "Reading char from file: \"hw6-2.in\"" << endl;
                    print = true;
                    pthread_mutex_unlock(&m_write);
                    pthread_cond_signal(&c_write);
                    pthread_cond_wait(&c2, &m2);
        ...
        while(1){
                if((fscanf(in, "%c\n",&resource)) != EOF){
                    cout << "Reading char from file: \"hw6-3.in\"" << endl;
                    print = true;
                    pthread_mutex_unlock(&m_write);
                    pthread_cond_signal(&c_write);
                    pthread_cond_wait(&c3, &m3);
